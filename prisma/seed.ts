...
// NOTE: This full seed (categories + suppliers) is for local/manual bootstrap
// only — it is NOT run automatically on deploy (see prisma/seed-categories.ts,
// which is what "npm run build" actually runs). Supplier data now comes from
// the Google Sheet sync endpoint (src/app/api/admin/suppliers/sync), so
// running this script against production would blow away sheet-driven edits.
// The suppliers below were originally sourced from a public research report
// on Cambodia hospitality suppliers and kept here as a historical bootstrap
